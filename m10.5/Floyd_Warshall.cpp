...
/*
Given a weighted directed graph, represented by an adjacency matrix dist[][] of size n x n, where dist[i][j] represents the weight of the edge from node i to node j. If there is no direct edge, dist[i][j] is set to a large value (i.e., 108) to represent infinity.
The graph may contain negative edge weights, but it does not contain any negative weight cycles.

Find the shortest distance between every pair of nodes i and j in the graph.
...
class Solution {
  public:
    void floydWarshall(vector<vector<int>> &dist) {
        const int mxNm = 100000000;
        
        for(int k = 0; k < dist.size(); k++)
            for(int i = 0; i < dist.size(); i++)
                for(int j = 0; j < dist.size(); j++)
                    if(dist[i][k] != mxNm && dist[k][j] != mxNm && dist[i][k] + dist[k][j] < dist[i][j])
